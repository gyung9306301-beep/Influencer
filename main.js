...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchSheetData() {
    ...
    setHtml(
      tableBody,
      `<tr><td colspan="11" class="loading">데이터를 불러오는 중입니다...</td></tr>`
    );
    ...
    try {
      ...
      setHtml(
        tableBody,
        `<tr><td colspan="11" class="empty">데이터를 불러오지 못했습니다.<br>시트가 공개 상태인지, 시트명이 정확히 <strong>Influencer list</strong>인지 확인해주세요.</td></tr>`
      );
  ...
  function applyFilters() {
    ...
    if (keyword) {
      rows = rows.filter((r) =>
        [r.B, r.C, r.D, r.E, r.F, r.G, r.H, r.I, r.J].some((v) =>
  ...
  function renderTable() {
    ...
    setText(planPill, `Current View: ${state.premiumPreview ? 'Premium Preview' : 'Free'}`);
    updateSelectionCount();

    if (!pageRows.length) {
      setHtml(tableBody, `<tr><td colspan="11" class="empty">조건에 맞는 데이터가 없습니다.</td></tr>`);
      return;
    }
    ...
    <tr>
      ...
      ${premiumHeaders
        .map(
          (key) =>
            `<td class="premium-col text-right ${state.premiumPreview ? 'premium-unlocked' : 'premium-locked'}">${renderCell(key, row[key])}</td>`
        )
        .join('')}
    </tr>
  `
  ...
  function togglePremiumPreview(event) {
    ...
    setText(previewPremiumBtn, state.premiumPreview ? 'Free 화면으로 보기' : '미리보기 전환');
    renderTable();
  }
